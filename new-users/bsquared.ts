...
const fetch = async (options: FetchOptions) => {
  ...
  const data = await httpGet(`https://www.oklink.com/api/explorer/v2/common/charts/uniqueAddress?chain=BSQUARED&t=${timestamp}`, { headers });
  const entry = data.data.value.find((item: any) => item.timestamp == timestamp);
  if (!entry) {
    throw new Error(`No BSquared new user data found for ${timestamp}`);
  }
  return {
